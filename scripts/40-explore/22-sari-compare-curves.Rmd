...
source(file.path(path_src, "visualize-curves.R"))
...
```{r}
ntimes <- nlevels(df_train$date)
names_lambda <- paste0("logN", 1:ntimes)

# predict
df_train <- df_train |>
    mutate(gomp_total = exp(coef(mod_gomp)[names_lambda])[date],
           gomp_q = cases / gomp_total,
           exp_total = exp(coef(mod_exp)[names_lambda])[date],
