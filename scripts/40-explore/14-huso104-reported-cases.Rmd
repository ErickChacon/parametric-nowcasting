...
huso <- readRDS(file.path(path_processed, "huso104_cum_2011_05_12_to_06_06.rds"))
```
...
```{r}
last_date <- max(huso$date)
huso_long <- reporcases_longer(huso) |>
    mutate(available_delay = as.numeric(last_date - date)) |>
